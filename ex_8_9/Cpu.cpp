...
void Cpu::processor_thread(void) {
	...
	while (true) {
        ...
        // measuring processing time to verify header integrity
        MEASURE_PROCESSING_TIME
        (
            // verifying packet header
            headerVerified = this->verifyHeaderIntegrity(this->m_packet_header);

            // delay for verifying header integrity: 50 cpu cycles
            wait(CLK_CYCLE_CPU * CPU_VERIFY_HEADER_CYCLES);
        );

        if (headerVerified) {

            // selection between using an accelerator or CPU
            if (use_accelerator) {
                // setting variables to be sent to accelerator
                this->m_lookup_request.destAddress = this->m_packet_header.getDestAddress();
                this->m_lookup_request.processorId = this->m_id;
                
                //*********************************************************
                // Writing the look up request to accelerator
                //*********************************************************  
                while (true) {
                    // sending lookup request to accelerator
                    MEASURE_TRANSFER_TIME
                    (
                    ...
                    if (tlm_stat == TLM_OK_RESPONSE) {
                        if (do_logging & LOG_CPU)
                            cout << sc_time_stamp() << " " << name() << ": Lookup request transaction finished successfully." << endl;
                    } else {
                        if (do_logging & LOG_CPU)
                            cout << sc_time_stamp() << " " << name() << ": Lookup request transaction failed." << endl;
                        
                        // Failed write to accelerator
                        continue;
                    }

                    if (do_logging & LOG_CPU)
                        cout << sc_time_stamp() << " " << name() << ": Wrote lookup request " << this->m_lookup_request <<" to accelerator." << endl;
                    
                    // successfull to write to accelerator
                    break;
                }

            } else {
                // measuring processing time for ip look up
                MEASURE_PROCESSING_TIME
                (
                    // Ip look up for forwarding port
                    port_id = this->makeNHLookup (this->m_packet_header);

                    // dealy for IPlookup: 350 cpu cycles
                    wait(CLK_CYCLE_CPU * CPU_IP_LOOKUP_CYCLES);
                );    
            }
            
            // measuring processing time for ttl decrement
            MEASURE_PROCESSING_TIME
            (
                // decrement TTL
                updatedTTL = this->decrementTTL(this->m_packet_header);

                // delay for decrement TTL: 5 cpu cycles
                wait(CLK_CYCLE_CPU * CPU_DECREMENT_TTL_CYCLES);
            );

            if (updatedTTL > 0) {
                // measuring processing time for updateChecksum
                MEASURE_PROCESSING_TIME
                (
                    // updating the checksum after decrementing TTL
                    this->updateChecksum(this->m_packet_header);
                ...
                // getting the port_id from the accelerator
                if (use_accelerator) {
                    // wait for look up interrupt to be ready
                    while (this->lookupReady_interrupt != true)
                        wait(this->lookupReady_interrupt.value_changed_event());
                    
                    //*********************************************************
                    // Reading the look up request to accelerator
                    //*********************************************************  
                    while (true) {
                        // sending lookup request to accelerator
                        MEASURE_TRANSFER_TIME
                        ...
                        if (do_logging & LOG_CPU)
                            cout << sc_time_stamp() << " " << name() << ": Read lookup request " << port_id << endl;
        ...
        if (discard_packet) {
            ...
            while (true) {
                ...
                // mesuring transfer time to write descriptor to output queue
                MEASURE_TRANSFER_TIME
                (
            ...
            while (true) {
                ...
                // measuring transfer time to write packet header to memory
                MEASURE_TRANSFER_TIME
                (
                    // starting transaction
