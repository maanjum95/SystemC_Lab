...
bool Cpu::read_new_packet_descriptor() 
{
    ...
    // measuring transfer time to read a new packet descriptor
...
bool Cpu::read_packet_header_from_mem() 
{
    ...
    // measuring transfer time to read packet header from memory
    MEASURE_TRANSFER_TIME 
    (
...
bool Cpu::process_packet_header(int &port_id)
{
    ...
    // measure processing time of verifying header integrity
    MEASURE_PROCESSING_TIME
    (
        // verifying packet header
        headerVerified = verifyHeaderIntegrity(this->m_packet_header);

        // delay for verifying header integerity: 50 cpu cycles
        wait(CLK_CYCLE_CPU * CPU_VERIFY_HEADER_CYCLES);
    );

    if (headerVerified) {
        if (use_accelerator) {
            // if Ip look up is performed via accelerator
            
            // get the destination address from ip header
            this->m_lookup_request.destAddress = this->m_packet_header.getDestAddress();

            // setting processor id of look up request to identify the cpu
            this->m_lookup_request.processorId = this->m_id;
            
            // measure transfer time to accelerator
            MEASURE_TRANSFER_TIME 
            (
                // writing the lookup request to the accelerator
                this->startTransaction(TLM_WRITE_COMMAND, ACCELERATOR_ADDRESS,
                                (unsigned char *) &this->m_lookup_request, 
                                sizeof(this->m_lookup_request));
            ...
            if (tlm_stat == TLM_OK_RESPONSE) {
                if (do_logging & LOG_CPU)
                    cout << sc_time_stamp() << " " << name() << ": Accelerator write transaction finished successfully." << endl;
            } else {
                if (do_logging & LOG_CPU)
                    cout << sc_time_stamp() << " " << name() << ": Accelerator write transaction failed." << endl;
            ...
            // measure processing time for ip look up on CPU
            MEASURE_PROCESSING_TIME
            (   
            // Ip look up to get output port computing on cpu
                port_id = this->makeNHLookup (this->m_packet_header);
    
                // delay for IPlookup: 350 cpu cycles
                wait(CLK_CYCLE_CPU * CPU_IP_LOOKUP_CYCLES);      
            );  
            
            if (do_logging & LOG_CPU)
                cout << sc_time_stamp() << " " << name() << ": NO ACCELERATOR: Packet needs to be forwarded to port: " << port_id << endl;  
        }

        // measure processing time for ttl decrement
        MEASURE_PROCESSING_TIME
        (   
            // decrementing TTL 
            decrementedTTL = decrementTTL(this->m_packet_header) ;
            
            // delay for decrement TLL: 5 cpu cycles
            wait(CLK_CYCLE_CPU * CPU_DECREMENT_TTL_CYCLES);
        );

        if (decrementedTTL > 0) {
            // measuring processing time to updateChecksum
            MEASURE_PROCESSING_TIME
            (
                // updating the checksum after decrementing TTL  
                updateChecksum(this->m_packet_header);

                // delay for calculation of updateChecksum: 30 cpu cycles
                wait(CLK_CYCLE_CPU * CPU_UPDATE_CHECKSUM_CYCLES);
            );

            if (use_accelerator) {
                // if we used an accelerator we need to get output port id from the accelerator
                wait(lookupReady_interrupt);

                // measure transfer time to accelerator
                MEASURE_TRANSFER_TIME 
                (
                    // writing the lookup request to the accelerator
                    this->startTransaction(TLM_READ_COMMAND, ACCELERATOR_ADDRESS,
                                    (unsigned char *) &this->m_lookup_request, 
                                    sizeof(this->m_lookup_request));
                ...
                if (tlm_stat == TLM_OK_RESPONSE) {
                    if (do_logging & LOG_CPU)
                        cout << sc_time_stamp() << " " << name() << ": Accelerator read transaction finished successfully." << endl;
                } else {
                    if (do_logging & LOG_CPU)
                        cout << sc_time_stamp() << " " << name() << ": Accelerator read transaction failed." << endl;

                    // failed to write the packet descrptor
                    return false;
                }

                // port id processed by the accelerator
                port_id = this->m_lookup_request.destAddress;

                if (do_logging & LOG_CPU)
                    cout << sc_time_stamp() << " " << name() << ": ACCELERATOR: Packet needs to be forwarded to port: " << port_id << endl; 
            }

            if (do_logging & LOG_CPU)
			    cout << sc_time_stamp() << " " << name() << ": Packet successfully processed. Ready to be forwarded to port: " << port_id << endl;
...
bool Cpu::write_descriptor_to_discard_queue() 
{
    ...
    // measuring transfer time to write descriptor to discard queue
    MEASURE_TRANSFER_TIME
    (
...
bool Cpu::write_packet_header_to_mem() 
{
    ...
    // measure transfer time to write packet header to memory
    MEASURE_TRANSFER_TIME
    (
        // starting transaction
