...
//*******================================================*******
tlm_sync_enum fifo_3::nb_transport_fw(tlm_generic_payload &payload,	// ref to Generic Payload 
	...
	if(phase != BEGIN_REQ) {
		cout << sc_time_stamp()
				<< " ERROR in FIFO: nb_transport_fw call with phase!=BEGIN_REQ"
				<< endl;
		exit(1);
	}
...
void fifo_3::do_read() {
	...
	while(true) {
		...
		else if(fill_level < (int)len) {	// not enough data to read
		...
		if(len > 0) {
			if(read_addr + (int)len < fifo_size)  // read in one chunk
		...
		if(tlm_resp != TLM_COMPLETED || phase != END_RESP) {
			cout << endl << sc_time_stamp()
					<< " fifo: read response not appropriately completed"
					<< endl;
		}
	}
}
...
void fifo_3::do_write() {
	...
	while(true) {
		...
		else if(fill_level + (int)len > fifo_size) { // not enough space for all data
		...
		if(len > 0) {
			if(write_addr + (int)len < fifo_size)  // write in one chunk
		...
		if(tlm_resp != TLM_COMPLETED || phase != END_RESP) {
			cout << endl << sc_time_stamp()
					<< " fifo: write response not appropriately completed"
					<< endl;
		}
	}
}
...
void fifo_3::output_fifo_status() {
	cout << "     ";
	cout << "Current fifo status: write address: " << write_addr
			<< ", read address: " << read_addr << ", fill level: " << fill_level
			<< endl;
	cout << "     ";
