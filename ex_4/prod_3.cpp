...
void prod_3::producer() {
	...
	while(true) {
		...
		if(pending_transaction) {
			cout << sc_time_stamp()
					<< " producer waiting until transaction is finished"
					<< endl;
			wait (response_event);
			...
			cout << endl << sc_time_stamp()
					<< " producer transaction is finished";
			...
			cout << "     producer wrote " << data_written << " of "
					<< data_length << " bytes." << endl;
		...
		// output message 
		cout << endl << sc_time_stamp() << " producer wants to write "
				<< data_length << " bytes: 0x ";
		...
		if(tlm_resp != TLM_UPDATED || phase != END_REQ) {
			cout << endl << sc_time_stamp()
					<< " producer: write request not appropriately completed"
					<< endl;
		}
	}  // end while true
}  // end producer 
...
//*******================================================*******
tlm_sync_enum prod_3::nb_transport_bw(tlm_generic_payload &payload, // ref to Generic Payload 
	...
	if(phase == BEGIN_RESP) {
		cout << endl << sc_time_stamp()
				<< " producer: write confirmation coming" << endl;
	}
	else {
		cout << endl << sc_time_stamp()
				<< " producer: write not correctly confirmed" << endl;
	}
