...
bool fifo_2::write_fifo(unsigned char *data, unsigned int &count) {
	...
	if(fill_level == fifo_size) // no free space
		len = 0;
	else if(fill_level + (int)count > fifo_size) // not enough space for all data
	...
	if(len > 0) {
		...
		cout << endl << sc_time_stamp() << " " << name()
				<< ": full - no data have been written" << endl;
...
bool fifo_2::read_fifo(unsigned char *data, unsigned int &count) {
	...
	if(fill_level == 0) // read attempt from empty FIFO
		len = 0;
	else if(fill_level < (int)count) // not enough data to read
	...
	if(len > 0) {
		...
		cout << endl << sc_time_stamp() << " " << name()
				<< ": empty - no data have been read" << endl;
...
void fifo_2::output_fifo_status() {
	cout << "     ";
	cout << "Current " << name() << " status: write address: " << write_addr
